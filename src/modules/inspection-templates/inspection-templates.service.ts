...
export class InspectionTemplatesService {
  ...
  /**
   * Pause an inspection (ONGOING → PAUSED)
   * Requirements: 2.2, 3.2, 5.1, 5.2, 5.3
   */
  async pauseInspection(inspectionId: string): Promise<WorkOrderInspectionResponse> {
    try {
      // Fetch inspection with current status
      const inspection = await this.prisma.workOrderInspection.findUnique({
        where: { id: inspectionId },
      ...
      // Validate state transition
      if (!isValidTransition(inspection.status as InspectionStatus, InspectionStatus.PAUSED)) {
        return {
          success: false,
          error: generateTransitionErrorMessage(
            inspectionId,
      ...
      const updated = await this.prisma.workOrderInspection.update({
        where: { id: inspectionId },
        data: { status: InspectionStatus.PAUSED },
        include: {
          template: {
            include: {
              templateItems: {
      ...
      return {
        ...
        message: 'Inspection paused successfully'
      };
    } catch (error) {
      console.error('Error pausing inspection:', error);
      return {
        success: false,
        error: 'Failed to pause inspection'
      };
    }
  }

  /**
   * Resume an inspection (PAUSED → ONGOING)
   * Requirements: 2.3, 3.3, 5.1, 5.2, 5.3
   */
  async resumeInspection(inspectionId: string): Promise<WorkOrderInspectionResponse> {
    try {
      // Fetch inspection with current status
      const inspection = await this.prisma.workOrderInspection.findUnique({
        where: { id: inspectionId },
      ...
      return {
        success: false,
        error: 'Failed to resume inspection'
      };
    }
  }
  ...
  /**
   * Cancel an inspection (PENDING/ONGOING/PAUSED → CANCELLED)
