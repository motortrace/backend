...
export class WorkOrderController {
  ...
  async createWorkOrderEstimate(req: Request, res: Response) {
    try {
      const { workOrderId } = req.params;
      const estimateData: CreateWorkOrderEstimateRequest = req.body;
      estimateData.workOrderId = workOrderId;

      const estimate = await this.workOrderService.createWorkOrderEstimate(estimateData);

      res.status(201).json({
        success: true,
        data: estimate,
        message: 'Work order estimate created successfully',
      });
    } catch (error: any) {
      res.status(400).json({
        success: false,
  ...
  async getWorkOrderEstimates(req: Request, res: Response) {
    try {
      const { workOrderId } = req.params;
      const estimates = await this.workOrderService.getWorkOrderEstimates(workOrderId);
  ...
  async approveWorkOrderEstimate(req: Request, res: Response) {
    try {
      const { workOrderId, estimateId } = req.params;
      const { approvedById } = req.body;

      const estimate = await this.workOrderService.approveWorkOrderEstimate(
        workOrderId,
        estimateId,
        approvedById
      );
      ...
      res.json({
        success: true,
        data: estimate,
        message: 'Work order estimate approved successfully',
      });
    } catch (error: any) {
      res.status(400).json({
        success: false,
  ...
  async createWorkOrderPart(req: Request, res: Response) {
    try {
      const { workOrderId } = req.params;
      const partData: CreateWorkOrderPartRequest = req.body;
      partData.workOrderId = workOrderId;

      const part = await this.workOrderService.createWorkOrderPart(partData);
